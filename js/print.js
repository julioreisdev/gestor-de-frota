...
export async function printList({ title, columns, rows, filtersLabel = '', colHints = [] }) {
  ...
  const logoUrl = reportLogoUrl(entity);
  const logoFallback = new URL('logo.png', location.href).href;
  const ibge = entity?.ibge_code || '';
  const total = rows.length;

  // Conteúdo gerado abaixo é injetado via openPrintTab (Blob URL → nova aba real).
...
<body>
  <div class="hdr">
    <img class="hdr-logo" src="${esc(logoUrl)}" alt="" onerror="this.onerror=null; this.src='${esc(logoFallback)}';">
    <div class="hdr-info">
  ...
  <div class="footer">
    Relatório gerado por <b>${esc(APP_NAME)}</b>
  </div>
  ...
  openPrintTab(html, { title, autoPrint: false /* já tem script inline */ });
}

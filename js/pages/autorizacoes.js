...
export async function renderFuelingTab(container) {
  container.innerHTML = `
    <div class="page-header" style="margin-bottom:var(--s-4)">
      ...
      <div class="page-actions">
        <button class="btn btn-outline btn-sm" id="btn-thermal-cfg" title="Tamanho da bobina térmica (58mm / 80mm)">⚙️ Bobina</button>
        ${isFornecedor() ? '' : `
  ...
  const btnCfg = document.getElementById('btn-thermal-cfg');
  if (btnCfg) btnCfg.addEventListener('click', () => askThermalWidth());
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  openPrintTab(html, { title: `Autorização ${a.number}` });
}
...
async function printAutThermal(id) {
  const a = _items.find(x => x.id === id);
  if (!a) return;
  if (!hasChosenThermalWidth()) {
    const picked = await askThermalWidth();
    if (!picked) return;
  }
  ...
  const bodyHTML = `
    <div class="center bold head-title">${esc(orgao)}</div>
    <div class="center head-sub">AUTORIZAÇÃO DE ABASTECIMENTO</div>
    <div class="sep"></div>
    ${cancel ? `<div class="cancel">CANCELADA</div>` : ''}
    <div class="center num">${esc(a.number)}</div>
    <div class="center date-line">${esc(fmtDate(a.date))} · ${esc(STATUS_LABEL[a.status].toUpperCase())}</div>
    <div class="sep"></div>
    <div class="row"><span class="lbl">Veículo:</span><span class="val">${esc(formatPlate(a.vehicle_plate_snapshot))}</span><span class="val-md">${esc(a.vehicle_model_snapshot)}</span></div>
    <div class="row"><span class="lbl">Secretaria:</span><span class="val-md">${esc(a.department_acronym_snapshot || '—')}</span></div>
    <div class="sep"></div>
    ...
    <div class="grid2">
      <div><span class="lbl">R$/L:</span><span class="val-md">${Number(a.unit_price_snapshot).toFixed(3)}</span></div>
      <div><span class="lbl">Total est:</span><span class="val">${fmtMoney(a.estimated_total)}</span></div>
    </div>
    <div class="sep"></div>
    ...
    <div class="qr-hint">Escaneie para verificar</div>
    ${!cancel ? `<div class="sign-block">Motorista</div><div class="sign-block">Frentista</div>` : ''}
    <div class="footer">Gerir Frota</div>
  `;
  await openThermalPDF(bodyHTML, { title: `Aut ${a.number}` });
}

    ...
    <div class="card report-filter-card">
      ...
      <div class="report-filter-grid">
        ...
        <div class="field">
          ...
          <select class="select" id="rf-type">
            <option value="">Abast. + Manut.</option>
            <option value="fueling">Só abastecimentos</option>
            <option value="maintenance">Só manutenções</option>
          </select>
        </div>
...
function renderSummary(k) {
  ...
  if (_filter.type === 'fueling')     parts.push('🎯 Só abastecimentos');
...
function filteredMaintenance() {
  if (_filter.type === 'fueling') return [];
...
function render() {
  ...
  // Toggle "manter total na exportação" (persistente em localStorage)
  const kp = document.getElementById('chk-keep-totals');
  if (kp) kp.addEventListener('change', () => setKeepTotals(kp.checked));
...
/** Preferência local: manter bloco de totalizações no PDF exportado. */
const KEEP_TOTALS_KEY = 'gf-report-keep-totals';
function getKeepTotals() {
  try { const v = localStorage.getItem(KEEP_TOTALS_KEY); return v === null ? true : v === '1'; }
  catch { return true; }
}
function setKeepTotals(v) {
  try { localStorage.setItem(KEEP_TOTALS_KEY, v ? '1' : '0'); } catch {}
}

/** Calcula totais gerais + por combustível para uma lista de abastecimentos. */
function computeAbastTotals(absList) {
  const nTotal = absList.length;
  let litrosTotal = 0, valorTotal = 0;
  const map = new Map(); // code → { code, description, n, litros, valor }
  for (const a of absList) {
    const litros = Number(a.quantity || 0);
    const valor  = Number(a.total || 0);
    litrosTotal += litros;
    valorTotal  += valor;
    const key = a.fuel_type_code;
    if (!map.has(key)) {
      map.set(key, {
        code: key,
        description: _fuels.find(f => f.code === key)?.description || `Cód ${key}`,
        n: 0, litros: 0, valor: 0,
      });
    }
    const g = map.get(key);
    g.n += 1; g.litros += litros; g.valor += valor;
  }
  const byFuel = [...map.values()].sort((a, b) => b.valor - a.valor);
  return { nTotal, litrosTotal, valorTotal, byFuel };
}

/** Bloco HTML de totalizações — usado tanto na tela quanto no PDF. */
function totalsBlockHTML(t, { pdf = false } = {}) {
  const wrap = pdf ? 'totals' : 'report-totals';
  const title = pdf ? 'totals-title' : 'report-totals-title';
  const grid = pdf ? 'totals-grid' : 'report-totals-grid';
  const item = pdf ? 'totals-item' : 'report-totals-item';
  const grand = pdf ? 'totals-item grand' : 'report-totals-item grand';
  const fmtL = n => n.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' L';
  const byFuelHTML = t.byFuel.length
    ? t.byFuel.map(f => `
        <div class="${item}">
          <div class="k">${esc(f.description)}</div>
          <div class="v">${fmtMoney(f.valor)}</div>
          <div class="sub">${f.n} abast. · ${fmtL(f.litros)}</div>
        </div>`).join('')
    : '';
  return `
    <div class="${wrap}">
      <div class="${title}">Totalizações</div>
      <div class="${grid}">
        ${byFuelHTML}
        <div class="${grand}">
          <div class="k">Total geral</div>
          <div class="v">${fmtMoney(t.valorTotal)}</div>
          <div class="sub">${t.nTotal} abast. · ${fmtL(t.litrosTotal)}</div>
        </div>
      </div>
    </div>`;
}
...
function tblAbastecimentosPorSec(absList) {
  ...
  const totals = computeAbastTotals(absList);
  const keep = getKeepTotals();
  const totalsHTML = totalsBlockHTML(totals);
  const toggleHTML = `
    <div class="report-totals-toggle">
      <label class="pref-row" style="padding:10px 12px;border-radius:var(--r-md);background:var(--surface-alt);border:1px solid var(--border);cursor:pointer">
        <input type="checkbox" id="chk-keep-totals" ${keep ? 'checked' : ''}>
        <div class="pref-text">
          <div class="pref-title" style="font-size:13px">Incluir totalizações no PDF exportado</div>
          <div class="pref-help" style="font-size:11.5px">Quando ativo, os totais por combustível e o total geral são acrescentados ao final do PDF da seção.</div>
        </div>
      </label>
    </div>
  `;

  return groupsHTML + totalsHTML + toggleHTML;
}
...
async function exportSection(sec, fmt) {
  ...
  if (fmt === 'xlsx') {
    ...
    // Totais na aba de Abastecimentos por Secretaria (Excel)
    if (sec === 'abast-detalhado' && getKeepTotals()) {
      const t = computeAbastTotals(filteredFueling());
      const startRow = data.rows.length + 3;
      XLSX.utils.sheet_add_aoa(ws, [
        ['TOTALIZAÇÕES'],
        ['Combustível', 'Abastecimentos', 'Litros', 'Valor (R$)'],
        ...t.byFuel.map(f => [f.description, f.n, +f.litros.toFixed(2), +f.valor.toFixed(2)]),
        ['TOTAL GERAL', t.nTotal, +t.litrosTotal.toFixed(2), +t.valorTotal.toFixed(2)],
      ], { origin: `A${startRow}` });
    }
    ...
    // Bloco de totalizações injetado no PDF quando pref ativa
    let extra = '';
    if (sec === 'abast-detalhado' && getKeepTotals()) {
      const t = computeAbastTotals(filteredFueling());
      extra = totalsBlockHTML(t, { pdf: true });
    }
    await exportPDFCustom(data.title, data.cols, data.rows, extra);
  }
}
...
function exportAllXLSX() {
  ...
  // Aba dedicada de totalizações (só se pref ativa e houver abastecimentos)
  if (getKeepTotals() && abs.length) {
    const t = computeAbastTotals(abs);
    const wsT = XLSX.utils.aoa_to_sheet([
      ['TOTALIZAÇÕES POR COMBUSTÍVEL'],
      [],
      ['Combustível', 'Abastecimentos', 'Litros', 'Valor (R$)'],
      ...t.byFuel.map(f => [f.description, f.n, +f.litros.toFixed(2), +f.valor.toFixed(2)]),
      ['TOTAL GERAL', t.nTotal, +t.litrosTotal.toFixed(2), +t.valorTotal.toFixed(2)],
    ]);
    wsT['!cols'] = [{ wch: 36 }, { wch: 18 }, { wch: 14 }, { wch: 16 }];
    XLSX.utils.book_append_sheet(wb, wsT, 'Totalizações');
  }
...
async function exportPDFCustom(title, cols, rows, extraHTML = '') {
  ...
  const html = pdfTemplate({ orgao, logo, ibge, period, entity_type: entity?.entity_type, body: `
...
async function exportPDF() {
  ...
  // Bloco de totalizações no fim (só se pref ativa e houver abastecimentos)
  const absList = filteredFueling();
  const totalsHTML = (getKeepTotals() && absList.length)
...
function pdfTemplate({ orgao, logo, ibge, period, entity_type, body }) {
  // Fallback local pra logo do Gerir Frota, caso o brasão remoto (S3, gstatic
  // etc) falhe ao carregar por CORS/404. Precisa ser URL absoluta pra funcionar
  // na aba Blob URL.
  const logoFallback = new URL('logo.png', location.href).href;
  const logoImg = logo
    ? `<img src="${esc(logo)}" alt="" onerror="this.onerror=null; this.src='${esc(logoFallback)}';">`
    : `<img src="${esc(logoFallback)}" alt="">`;
  return `<!DOCTYPE html>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  h2.section { font-size:13px; font-weight:700; color:#1A65B5; margin:18px 0 8px; padding-bottom:4px; border-bottom:1px solid #E5E9F0; page-break-after:avoid; break-after:avoid-page; }
  /* Tabela: NÃO usar page-break-inside:avoid — força página em branco quando
     a tabela é maior que a folha. Repetir cabeçalho em cada página impressa. */
  ...
  /* Bloco de totalização (aparece após tabelas quando incluído) */
  .totals { margin-top:12px; page-break-inside:avoid; break-inside:avoid; }
  .totals-title { font-size:10px; font-weight:700; color:#0F172A; text-transform:uppercase; letter-spacing:.04em; margin-bottom:6px; }
  .totals-grid { display:grid; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); gap:6px; }
  .totals-item { border:1px solid #E5E9F0; border-left:3px solid #16A34A; border-radius:5px; padding:6px 10px; background:#F8FAFC; }
  .totals-item .k { font-size:9px; color:#64748B; font-weight:600; text-transform:uppercase; letter-spacing:.04em; }
  .totals-item .v { font-size:12px; font-weight:700; color:#0F172A; margin-top:2px; }
  .totals-item .sub { font-size:9px; color:#64748B; margin-top:2px; }
  .totals-item.grand { border-left-color:#217AD8; background:#EAF2FB; }
  .totals-item.grand .v { color:#1A65B5; font-size:13px; }
  ...
  ${body}
  <div class="footer">Relatório gerado por Gerir Frota</div>
